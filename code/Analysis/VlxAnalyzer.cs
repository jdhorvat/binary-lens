...
public static class VlxAnalyzer
{
    ...
    private static readonly Dictionary<byte, string> Opcodes = new()
    {
        ...
        { 0x21, "END DEFUN CLN"  },  // end defun + cleanup N local vars (2-byte count)
        ...
        { 0x26, "SUB (FSL)"      },  // FSL subtraction (pop 2, push result)
        { 0x27, "STOP"           },
        ...
        { 0x2B, "STOP"           },
        { 0x2C, "INDEX8"          },  // indexed access (1-byte index, pops 1)
        { 0x2D, "ASSIGN8"         },  // indexed assignment (1-byte index, pops 2)
        ...
        { 0x4F, "1+"             },  // increment by 1 (unary)
        { 0x50, "1-"             },  // decrement by 1 (unary)
        ...
        { 0x65, "TRACE-IN"       },  // debug trace entry marker
        { 0x66, "TRACE-OUT"      },  // debug trace exit marker
    ...
    private static void InterpretResourceStream(
        ...
        Dictionary<int, string> mVarsRes,    // table 0: resource stream vars
        Dictionary<int, string> mVarsFunc,   // table 1: function stream vars
        ...
        while (pos < resStream.Length)
        {
            ...
            switch (opcode)
            {
                ...
                case 0x43:  // INIT vars
                {
                    ...
                    // Pop module ID: "nil" or empty/? → table 0 (resource), "0" or numeric → table 1 (function)
                    string moduleId = stack.Count > 0 ? stack.Pop() : "?";
                    bool isResourceTable = (moduleId == "nil" || moduleId == "?" || moduleId == "" || moduleId == "NIL");

                    // Pop count items from stack, store into appropriate table
                    for (int i = count - 1; i >= 0; i--)
                    {
                        if (stack.Count > 0)
                        {
                            ...
                            if (isResourceTable)
                                mVarsRes[startIdx + i] = val;
                            else
                                mVarsFunc[startIdx + i] = val;
                        }
                    }
                    break;
                }
                ...
                case 0x09:  // PUSH gvar
                {
                    ...
                    pos += 2;
                    string val = (mVarsFunc.ContainsKey(idx) ? mVarsFunc[idx]
                                : mVarsRes.ContainsKey(idx) ? mVarsRes[idx]
                                : $"#{idx}");
                    stack.Push(val);
                    break;
                }
                ...
                case 0x06:  // SETQ
                {
                    ...
                    {
                        string val = stack.Pop();
                        mVarsRes[idx] = val;  // resource stream SETQ goes to resource table
                    }
                    break;
                }
                ...
                case 0x35:  // ld_USUBR
                {
                    ...
                    string funcRef = (mVarsFunc.ContainsKey(gvarIdx) ? mVarsFunc[gvarIdx]
                                    : mVarsRes.ContainsKey(gvarIdx) ? mVarsRes[gvarIdx]
                                    : $"#{gvarIdx}");
                    stack.Push(funcRef);
                    break;
                }
    ...
    private static void DisassembleFas(VlxInfo info)
    {
        ...
        // ── Module Variable Table (TWO-TABLE MODEL) ────────────────────
        // In FAS, the symbol/string table is built DURING bytecode interpretation:
        // - mVarsRes (table 0): symbols/strings from resource stream INIT (moduleId=nil)
        // - mVarsFunc (table 1): symbols/strings from function stream INIT (moduleId≠nil)
        ...
        //   0x43 (INIT vars) pops items from the stack into mVarsRes or mVarsFunc[startIdx..startIdx+count-1]
        // Then opcodes 0x03, 0x06, 0x09, 0x35, 0x51 etc. reference them, preferring mVarsFunc.
        var mVarsRes = new Dictionary<int, string>();    // table 0: resource stream variable table
        var mVarsFunc = new Dictionary<int, string>();   // table 1: function stream variable table
        var funcNames = new Dictionary<int, string>();   // function offset → name

        // If a resource stream is available, interpret it first to populate both mVars tables
        if (info.ResStreamData != null && info.ResStreamData.Length > 0)
        {
            InterpretResourceStream(info.ResStreamData, fasVer, mVarsRes, mVarsFunc, funcNames);
        }
        ...
        // Stack for nested function contexts (for nested DEFUN support)
        var funcStack = new Stack<(FasFunction func, List<string> locals, int indent, StringBuilder lisp)>();

        // ── progn / branch tracking ─────────────────────────────────
        // We use markers in the LISP output that get replaced in post-processing:
        //   ‹PROGN_START:N› — placeholder at the start of an if-branch
        //   ‹PROGN_END:N›   — placeholder at the end of an if-branch
        // Each branch gets a unique ID N. During post-processing, if a branch
        // had multiple statements, the markers become (progn ...) wrapping.
        // If single statement, the markers are simply removed.
        int branchIdCounter = 0;
        // Stack of (branchId, indent) for open if-branches
        var ifBranchIds = new Stack<int>();
        // Count of statements emitted per branch ID
        var branchStmtCounts = new Dictionary<int, int>();
        // Pending if-close: offset → indent level to close at that offset
        // When the else-branch ends (at the JMP target), we need to close the if.
        var pendingIfCloses = new SortedDictionary<int, List<int>>();
        string? lastSetqValue = null;  // for suppressing duplicate return values
        ...
        // Helper: resolve module variable by index, preferring function stream table, translating internal names
        string ResolveMVar(int idx)
        {
            // Check mVarsFunc first (function stream symbols take priority), then mVarsRes
            string? name = null;
            if (mVarsFunc.TryGetValue(idx, out var funcVar))
                name = funcVar;
            else if (mVarsRes.TryGetValue(idx, out var resVar))
                name = resVar;
        ...
        while (pos < code.Length && instrCount < MaxInstrs)
        {
            ...
            // ── Check for pending if-else closes at this offset ──────
            // When we reach the target of a forward JMP from a then-branch,
            // the else-branch ends here — close the if block.
            if (pendingIfCloses.Count > 0)
            {
                // Process all closes at or before this offset
                var closesToProcess = new List<int>();
                foreach (var kvp in pendingIfCloses)
                {
                    if (kvp.Key > offset) break;
                    closesToProcess.Add(kvp.Key);
                }
                // Only flush and close if there are actually closes due at this offset
                if (closesToProcess.Count > 0)
                {
                // Flush any values left on the stack as statements BEFORE
                // closing the else-branch. These are side-effect calls
                // (e.g. 2DTEXT->3DTEXT, ENTDEL) whose results were left
                // on the stack inside the branch.
                while (stack.Count > 0)
                {
                    string leftover = StackPop();
                    if (leftover != "?" && leftover != "nil")
                    {
                        lispSb.AppendLine($"{Indent()}{leftover}");
                        if (ifBranchIds.Count > 0)
                            branchStmtCounts[ifBranchIds.Peek()]++;
                    }
                }

                foreach (var closeOff in closesToProcess)
                {
                    foreach (int bid in pendingIfCloses[closeOff])
                    {
                        // Close else-branch progn marker
                        lispSb.AppendLine($"‹PROGN_END:{bid}›");
                        // Close the if block
                        if (indentLevel > 0)
                        {
                            indentLevel--;
                    ...
                    pendingIfCloses.Remove(closeOff);
                }
                } // end if closesToProcess.Count > 0
            }
            ...
            // No command merging buffer anymore — each expression is emitted independently
            ...
            switch (op)
            {
                ...
                case 0x0A: // POP — discard top of stack and emit as statement
                {
                    ...
                    {
                        lastSetqValue = null;  // POP resets the setq tracking
                        // Emit the expression directly without merging consecutive (command ...) calls
                        // Each (command ...) from the bytecode is independent
                        lispSb.AppendLine($"{Indent()}{discarded}");
                        // Track statement count for progn detection
                        if (ifBranchIds.Count > 0)
                            branchStmtCounts[ifBranchIds.Peek()]++;
                    }
                    break;
                ...
                case 0x16: // END defun
                    if (currentFunc != null)
                    {
                        ...
                        // If the return value matches the last setq'd value, suppress it
                        // (DUP + SETQ pattern: value is both stored and returned).
                        string retVal = stack.Count > 0 ? StackPop() : "nil";
                        if (retVal == lastSetqValue)
                            lispSb.AppendLine($"{Indent()})");  // just close, don't repeat value
                        else
                        ...
                        // Check if we have a parent function (nested DEFUN case)
                        if (funcStack.Count > 0)
                        {
                            // Pop the outer function context
                            var (outerFunc, outerLocals, outerIndent, outerLisp) = funcStack.Pop();
                            // Append the nested function's LISP to the outer function's buffer
                            outerLisp.Append(lispSb);
                            lispSb = outerLisp;  // restore outer buffer (now includes nested output)
                            currentFunc = outerFunc;
                            localVarNames = outerLocals;
                            indentLevel = outerIndent;
                        }
                        else
                        {
                ...
                case 0x65: case 0x66: // TRACE-IN / TRACE-OUT (debug markers, no operands)
                    break;
                case 0x23: // NULL/NOT
                {
                    string arg = StackPop();
                ...
                case 0x64: // CLR lvar8 — clear local var (8-bit index)
                    if (pos < code.Length)
                    {
                        ...
                        else if (op == 0x08)
                        {
                            ...
                            lastSetqValue = v;
                            if (ifBranchIds.Count > 0) branchStmtCounts[ifBranchIds.Peek()]++;
                        }
                    ...
                    if (pos + 1 < code.Length)
                    {
                        ...
                        if (op is 0x0D or 0x3C) // BRZ — conditional
                        {
                            ...
                            else
                            {
                                ...
                                indentLevel++;
                                int bid16 = branchIdCounter++;
                                ifBranchIds.Push(bid16);
                                branchStmtCounts[bid16] = 0;
                                lispSb.AppendLine($"‹PROGN_START:{bid16}›");
                            }
                        }
                    }
                    break;
                ...
                case 0x57: // JMP 32/16 (far goto)
                {
                    ...
                    if (ok)
                    {
                        ...
                        // Flush any values left on the stack as statements.
                        // These are side-effect calls (entdel, 2DText->3DText, etc.)
                        // whose results were left on the stack inside a branch.
                        while (stack.Count > 0)
                        {
                            string leftover = StackPop();
                            if (leftover != "?" && leftover != "nil")
                        ...
                        {
                            // JMP +0 or +1 = end of single-branch if (no else)
                            // Close the progn marker for this branch
                            if (ifBranchIds.Count > 0)
                            {
                                int bid = ifBranchIds.Pop();
                                lispSb.AppendLine($"‹PROGN_END:{bid}›");
                            }
                            if (indentLevel > 0)
                            {
                        ...
                        {
                            // Check if this JMP skips over an inline DEFUN body.
                            // Pattern: JMP +N where within the next few bytes there's
                            // a DEFUN opcode (0x14 or 0x15). This is NOT an if/else
                            // transition — it's the compiler skipping the defun body
                            // so it's only entered when called.
                            bool skipsOverDefun = false;
                            int scanEnd = Math.Min(pos + 6, target); // DEFUN is typically 1-5 bytes after JMP
                            for (int scanPos = pos; scanPos < scanEnd && scanPos < code.Length; scanPos++)
                            {
                                byte scanOp = code[scanPos];
                                if (scanOp == 0x14 || scanOp == 0x15)
                                {
                                    skipsOverDefun = true;
                                    break;
                                }
                            ...
                            if (skipsOverDefun)
                            {
                                // This JMP skips over an inline defun body — not an if/else.
                                // Just close the current block if we're in one.
                                // Don't emit progn markers or register pending closes.
                            }
                            else
                            {
                            // Forward jump with offset > 1 = end of then-branch, else follows
                            // Close the then-branch progn marker
                            if (ifBranchIds.Count > 0)
                            {
                                int bid = ifBranchIds.Pop();
                                lispSb.AppendLine($"‹PROGN_END:{bid}›");
                            }
                            // Start a new progn marker for the else-branch
                            int elseBid = branchIdCounter++;
                            ifBranchIds.Push(elseBid);
                            branchStmtCounts[elseBid] = 0;
                            lispSb.AppendLine($"‹PROGN_START:{elseBid}›");
                            // Register a pending close for when we reach the JMP target
                            // (the else-branch ends at the target offset)
                            if (!pendingIfCloses.ContainsKey(target))
                                pendingIfCloses[target] = new List<int>();
                            pendingIfCloses[target].Add(elseBid);
                            } // end else (not skipsOverDefun)
                        }
                    }
                    break;
                }
                    ...
                    if (ok)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                lispSb.AppendLine($"{Indent()}(if {cond}");
                                indentLevel++;
                                // Start progn tracking for this if-branch
                                int bid = branchIdCounter++;
                                ifBranchIds.Push(bid);
                                branchStmtCounts[bid] = 0;
                                lispSb.AppendLine($"‹PROGN_START:{bid}›");
                            }
                        }
                    }
                    break;
                ...
                case 0x15: // DEFUN-Q
                    if (pos + 3 < code.Length)
                    {
                        ...
                        // If we're already inside a function, save the current state (nested DEFUN)
                        if (currentFunc != null)
                        {
                            funcStack.Push((currentFunc, localVarNames, indentLevel, lispSb));
                            lispSb = new StringBuilder(4096);  // fresh buffer for nested function
                            indentLevel++;  // further indent for nested function
                        }
                        ...
                        lastSetqValue = null;
                        if (funcStack.Count == 0)
                            indentLevel = 1;  // top-level function starts at indent 1
                        string keyword = op == 0x15 ? "defun-q" : "defun";
                        lispSb.AppendLine($"{Indent()}({keyword} {funcName} (__DEFUN_ARGS__)");
                        lispSb.AppendLine($"{Indent()}  ; locals={localVars} args={p2}..{p3}");
                    }
                    break;
                ...
                case 0x17:
                    if (pos + 1 < code.Length)
                    {
                        ...
                        if (funcStack.Count == 0)
                            indentLevel = 1;
                        lispSb.AppendLine($"{Indent()}(defun {name} (__DEFUN_ARGS__)");
                    }
                    break;
                ...
                // then pops St_init items from stack into mVarsFunc or mVarsRes[VarPos..VarPos+St_init-1]
                case 0x43:
                    if (pos + 3 < code.Length)
                    {
                        ...
                        // Pop module ID from stack: "nil" or empty → table 0 (resource), numeric → table 1 (function)
                        string moduleId = StackPop();
                        bool isResourceTable = (moduleId == "nil" || moduleId == "?" || moduleId == "" || moduleId == "NIL");

                        operands = $"start={varPos} count={stInit} module={moduleId}";

                        // Pop stInit items from stack into mVarsFunc or mVarsRes (reverse order)
                        ...
                        sb.AppendLine($"; Module vars populated: mVarsRes={mVarsRes.Count} mVarsFunc={mVarsFunc.Count} total entries");
                    }
                    break;
                ...
                case 0x4D: // >
                case 0x4E: // !=
                case 0x4F: // 1+ (increment by 1, unary)
                case 0x50: // 1- (decrement by 1, unary)
                {
                    ...
                    if (op is 0x4F or 0x50) // unary: 1+, 1-
                    {
                        string a = StackPop();
                        stack.Push($"({lispOp} {a})");
                    }
                ...
                // ── Indexed access/assignment (1-byte index operand) ─────
                case 0x2C: // INDEX8 — indexed access (1-byte index, pops 1, pushes result)
                    if (pos < code.Length)
                    {
                        byte idx8 = code[pos++];
                        string src = StackPop();
                    ...
                    break;
                case 0x2D: // ASSIGN8 — indexed assignment (1-byte index, pops 2)
                    if (pos < code.Length)
                    {
                        byte idx8 = code[pos++];
                        string val2 = StackPop(), target2 = StackPop();
                        operands = $"[{idx8}] {target2} = {val2}";
                        stack.Push($"(setf {target2}[{idx8}] {val2})");
                    }
                    break;

                // ── END DEFUN cleanup (consumes 2-byte var count) ─────────
                case 0x21:
                    if (pos + 1 < code.Length)
                    {
                        ushort cleanupCount = BitConverter.ToUInt16(code, pos); pos += 2;
                        operands = $"cleanup={cleanupCount} vars";
                        // Acts as end-defun with local var cleanup
                        if (currentFunc != null)
                        {
                            string retVal = stack.Count > 0 ? StackPop() : "nil";
                            if (retVal == lastSetqValue)
                ...
                // ── FSL subtraction (pop 2, push difference) ────────────
                case 0x26:
                {
                    string bv = StackPop(), a = StackPop();
                    stack.Push($"(- {a} {bv})");
        ...
        sb.AppendLine($"; ── End  ({instrCount} instructions decoded, {mVarsRes.Count + mVarsFunc.Count} module vars) ──");
        ...
        if (lispSb.Length > 100) // only if we actually reconstructed something
        {
            ...
            // ── Post-process progn markers ──────────────────────────────
            // Replace ‹PROGN_START:N›...‹PROGN_END:N› markers:
            //   If the branch had > 1 statement, wrap contents in (progn ...)
            //   If 0 or 1 statement, just remove the markers
            string lispText = lispSb.ToString();
            for (int bid = 0; bid < branchIdCounter; bid++)
            {
                string startMarker = $"‹PROGN_START:{bid}›";
                string endMarker   = $"‹PROGN_END:{bid}›";
                int stmtCount = branchStmtCounts.TryGetValue(bid, out var sc) ? sc : 0;

                if (stmtCount > 1)
                {
                    // Multi-statement branch: wrap in (progn ...)
                    // Find indentation of the start marker line to produce matching indent
                    int startIdx = lispText.IndexOf(startMarker);
                    if (startIdx >= 0)
                    {
                        // Find the indent before the start marker
                        int lineStart = lispText.LastIndexOf('\n', startIdx);
                        string indent = "";
                        if (lineStart >= 0)
                        {
                            // The marker is on its own line — use the previous line's indentation
                            // Actually, just use 2 spaces per indent level approximation
                        }
                        lispText = lispText.Replace(startMarker + "\n", "(progn\n");
                        lispText = lispText.Replace(startMarker + "\r\n", "(progn\r\n");
                        lispText = lispText.Replace(startMarker, "(progn");
                        lispText = lispText.Replace(endMarker + "\n", ") ; end progn\n");
                        lispText = lispText.Replace(endMarker + "\r\n", ") ; end progn\r\n");
                        lispText = lispText.Replace(endMarker, ") ; end progn");
                    }
                }
                else
                {
                    // Single or no statement: just remove markers
                    lispText = lispText.Replace(startMarker + "\n", "");
                    lispText = lispText.Replace(startMarker + "\r\n", "");
                    lispText = lispText.Replace(startMarker, "");
                    lispText = lispText.Replace(endMarker + "\n", "");
