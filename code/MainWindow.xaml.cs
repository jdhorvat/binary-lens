...
public partial class MainWindow : Window
{
    ...
    private void PopulateVlx(AnalysisResult r)
    {
        ...
        if (disasmModuleCount > 1)
        {
            ...
            BtnCopySource.IsEnabled = true;
            BtnSaveAll.Visibility   = hasLisp ? Visibility.Visible : Visibility.Collapsed;
            BtnSaveAll.IsEnabled    = hasLisp;
        }
    ...
    private void SaveSource_Click(object sender, RoutedEventArgs e)
    {
        ...
        bool isVlx    = _result.IsVlx && _result.VlxInfo != null
                      && !string.IsNullOrEmpty(_result.VlxInfo.Disassembly);

        string title, filter, defaultName;
        if (isDotNet)
        {
            title       = "Save Decompiled Source";
            filter      = "C# Source|*.cs|Text|*.txt";
            defaultName = Path.GetFileNameWithoutExtension(_result.FileName) + "_decompiled.cs";
        }
        else if (isVlx)
        {
            title       = "Save FAS Bytecode Disassembly";
            filter      = "Text|*.txt|ASM|*.asm";
            defaultName = Path.GetFileNameWithoutExtension(_result.FileName) + "_disasm.txt";
        }
        else
        {
        ...
        try
        {
            ...
            {
                // FAS bytecode disassembly is stored as a pre-formatted string
                File.WriteAllText(dlg.FileName, _result.VlxInfo!.Disassembly!,
                                  System.Text.Encoding.UTF8);
            }
            else
            {
    ...
    private void SaveLisp_Click(object sender, RoutedEventArgs e)
    {
        ...
        // Collect all available LISP sources: per-module first, fall back to top-level aggregate
        ...
        // Only add the top-level aggregate if no individual modules were found
        // (avoids showing a bogus "filename" entry that's just all modules concatenated)
        if (sources.Count == 0 && !string.IsNullOrEmpty(vlx.DecompiledLisp))
    ...
    {
        var vlx = _result?.VlxInfo;
        if (vlx == null || _result == null) return;

        var folderDlg = new OpenFolderDialog
        {
            Title = "Select Destination Folder for All Output Files",
        };
        if (folderDlg.ShowDialog() != true) return;
        ...
        try
        {
            // Save disassembly — per-module if available, else top-level aggregate
            bool savedModuleDisasm = false;
            foreach (var mod in vlx.Modules)
            {
                if (!string.IsNullOrEmpty(mod.Disassembly))
                {
                    string safeName = string.Join("_", mod.Name.Split(Path.GetInvalidFileNameChars()));
            ...
            {
                string path = Path.Combine(folder, baseName + "_disasm.txt");
                File.WriteAllText(path, vlx.Disassembly, System.Text.Encoding.UTF8);
                saved++;
            }

            // Save decompiled LISP — per-module if available, else top-level aggregate
            ...
            {
                string path = Path.Combine(folder, baseName + "_decompiled.lsp");
                File.WriteAllText(path, vlx.DecompiledLisp, System.Text.Encoding.UTF8);
                saved++;
            }

            ShowStatus($"Saved {saved} file(s) to {folder}");
        }
        catch (Exception ex)
        {
            MessageBox.Show($"Save failed: {ex.Message}", "BinaryLens",
    ...
    {
        if (_result == null || _result.Strings.Count == 0) return;

        var dlg = new SaveFileDialog
        {
            Title    = "Export Strings",
            Filter   = "CSV|*.csv|Tab-separated|*.tsv|Text (one per line)|*.txt",
            FileName = Path.GetFileNameWithoutExtension(_result.FileName) + "_strings.csv",
        };
        if (dlg.ShowDialog() != true) return;
        ...
        try
        {
            ...
            {
                sw.WriteLine($"Offset{sep}Encoding{sep}Value");
                foreach (var s in _result.Strings)
                {
                    // Escape CSV values: quote if it contains separator, quotes, or newlines
                    string val = s.Value.Replace("\"", "\"\"");
                    if (val.Contains(sep) || val.Contains('"') || val.Contains('\n') || val.Contains('\r'))
                        val = $"\"{val}\"";
                    sw.WriteLine($"{s.OffsetHex}{sep}{s.Encoding}{sep}{val}");
                }
            }
            else
            {
                // Plain text — just values, one per line
                foreach (var s in _result.Strings)
                    sw.WriteLine(s.Value);
            }

            ShowStatus($"Exported {_result.Strings.Count:N0} strings to {dlg.FileName}");
        }
        catch (Exception ex)
        {
            MessageBox.Show($"Export failed: {ex.Message}", "BinaryLens",
    ...
    private void ExportResources_Click(object sender, RoutedEventArgs e)
    {
        if (_result == null || _result.Resources.Count == 0) return;

        var dlg = new SaveFileDialog
        {
            Title    = "Export Resources List",
            Filter   = "CSV|*.csv|Tab-separated|*.tsv|Text|*.txt",
            FileName = Path.GetFileNameWithoutExtension(_result.FileName) + "_resources.csv",
        };
        if (dlg.ShowDialog() != true) return;
        ...
        try
        {
            ...
            if (ext is ".csv" or ".tsv")
            {
                sw.WriteLine($"Type{sep}Name{sep}Language{sep}Size");
                foreach (var r in _result.Resources)
                {
                    string name = (r.Name ?? "").Replace("\"", "\"\"");
                    if (name.Contains(sep) || name.Contains('"'))
                        name = $"\"{name}\"";
                    sw.WriteLine($"{r.Type}{sep}{name}{sep}{r.Language}{sep}{r.Size}");
                }
            }
            else
            {
                foreach (var r in _result.Resources)
                    sw.WriteLine($"{r.Type}\t{r.Name}\t{r.Language}\t{r.Size}");
            }

            ShowStatus($"Exported {_result.Resources.Count:N0} resources to {dlg.FileName}");
        }
        catch (Exception ex)
        {
            MessageBox.Show($"Export failed: {ex.Message}", "BinaryLens",
